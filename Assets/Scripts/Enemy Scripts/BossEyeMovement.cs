...
public class BossEyeMovement : MonoBehaviour
{
    ...
    public float[] fireballSpeed = { 2.5f, -2.5f };  // Скорость вращения огненных шаров
    public float distance = 0.25f;                  // Расстояние от огненных шаров до босса
    public Transform[] fireballs;                   // Массив огненных шаров
    ...
    private void Update()
    {
        ...
        // Вращение огненных шаров
        for (int i = 0; i < fireballs.Length; i++)
        {
            fireballs[i].position = transform.position + new Vector3(
                Mathf.Cos(Time.time * fireballSpeed[i]) * distance,
